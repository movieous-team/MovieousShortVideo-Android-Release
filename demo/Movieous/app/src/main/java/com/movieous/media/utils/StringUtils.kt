...
object StringUtils {
    ...
    val currentTimeString: String
        get() = DATE_FORMAT_PART.format(Calendar.getInstance().time)
     ...
     */
    val date: String
        get() = formatDate(Date(), DEFAULT_DATE_PATTERN)
     ...
     */
    val dateTime: String
        get() = formatDate(Date(), DEFAULT_DATETIME_PATTERN)
     ...
     */
    @JvmOverloads
    fun formatDate(date: Date, pattern: String = DEFAULT_DATE_PATTERN): String {
    ...
    fun generateTime(time: Long): String {
        ...
        return if (hours > 0)
            ...
            if (minutes > 0)
                ...
                if (totalSeconds > 0)
                    String.format("%02d''%02d", totalSeconds, millisSeconds) // 显示秒和毫秒
                else
    ...
    fun generateMillisTime(totalMillisSeconds: Int): String {
        ...
        val minutes = totalSeconds / 60 % 60
        return if (minutes > 0)
            String.format("%02d:%02d", minutes, seconds)  //分钟和秒
        else
            if (seconds > 0)
                String.format("%02d''%02d", seconds, millisSeconds) // 秒和毫秒
            else
