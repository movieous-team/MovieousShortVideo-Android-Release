...
public class HorizontalIndicatorView extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        TintTypedArray tta = TintTypedArray.obtainStyledAttributes(getContext(), attrs, R.styleable.HorizontalIndicatorView);
        //两种字体颜色和字体大小
        ...
        mSelectedColor = tta.getColor(R.styleable.HorizontalIndicatorView_SelectedTextColor, mContext.getResources().getColor(android.R.color.black));
        mTextSize = tta.getFloat(R.styleable.HorizontalIndicatorView_TextSize, 40);
        mTextColor = tta.getColor(R.styleable.HorizontalIndicatorView_TextColor, mContext.getResources().getColor(android.R.color.darker_gray));
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (scrollX > mPreviewX && scrollX - mPreviewX >= mFontSize && mCurrentIndex > 0) {
                    mOffset = 0;
                ...
                } else if (mPreviewX - scrollX >= mFontSize && mCurrentIndex < mIndicators.size() - 1) {
                    mOffset = 0;
                    mCurrentIndex = mCurrentIndex + 1;
                    mPreviewX = scrollX;
                    if (mOnIndicatorListener != null) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCurrentIndex >= 0 && mCurrentIndex <= mIndicators.size() - 1) {
            ...
            for (int i = 0; i < mIndicators.size(); i++) {
                ...
                if (i != mCurrentIndex) {
                    canvas.drawText(mIndicators.get(i), (i - mCurrentIndex) * mFontSize + getWidth() / 2 - mTextWidth / 2 + mOffset, getHeight() / 2 + mTextHeight / 2, mTextPaint);
                }
            }
        }
    }
